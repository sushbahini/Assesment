...
task2="curl https://www.thegazette.co.uk/all-notices/notice/data.json?start-publish-date=2020-01-01\&end-publish-date=2020-03-31\&results-page"  
...
def task1_2linux_way():
    json_string = subprocess.check_output(task2, shell=True)
    ...
    for x in range(1, int(last_page_number)):
        ...
        json_string = subprocess.check_output(task2+"="+str(x)+" | jq \".entry\"", shell=True)
        json_dictionary = json.loads(json_string)
        for dict in json_dictionary:
            if 'f:notice-code' in dict:
                ...
                if dict['f:notice-code'] == "2901":
                    print(str(dict))
                    with open('data_task1_2.json') as json_file:
                        print("data_task1_2.json")
                        ...
                        if duplicate == False:
                            temp.append(dict)
                    write_json(data,"data_task1_2.json")
        time.sleep(5)
    ...
    with open(file1) as json_file:
        ...
        with open(file2) as json_file1:
            ...
            for entry in data2['entries']:
                temp.append(entry)
    write_json(data_task,"data_task_2.json")
...
#joinDataFiles("data_task1_2.json","2020.json")
