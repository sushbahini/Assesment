...
task4="curl https://www.thegazette.co.uk/insolvency/notice/data.json?start-publish-date=2015-01-01\&end-publish-date=2015-03-31\&results-page"
...
and then get all entries and do all necessary checks to avoid data duplicacy and retrieve only needed data and follow the data governance like when looping have a sleep for more than 1 sec.
'''

def task4_linux_way():
    #Making a curl request to get the first page with given links for all the resultsets
    json_string = subprocess.check_output(task4, shell=True)
    json_dictionary = json.loads(json_string)

    # Getting the last page number from above output
    for dict in json_dictionary['link']:
        #Below check for the last link with the type json and fetches the last number for the link
    ...
    for x in range(1, int(last_page_number)):
        ...
        json_string = subprocess.check_output(task4+"="+str(x)+" | jq \".entry\"", shell=True)
        json_dictionary = json.loads(json_string)

        #Below checks and retrives notice feed for the given notice code in the list
        list_code = ["2432","2431","2433","2443","2441"]
        for dict in json_dictionary:
            if 'f:notice-code' in dict:
                print(str(dict['f:notice-code']))
                if dict['f:notice-code'] in list_code:
                    print(str(dict))
                    with open('data_task1_4.json') as json_file:
                        print("data_task1_4.json")
                        ...
                        if duplicate == False:
                            temp.append(dict)
                    write_json(data,"data_task1_4.json")
        time.sleep(5)
...
def joinDataFiles(file1,file2):
    with open(file1) as json_file:
        data_task1_4 = json.load(json_file)
        temp = data_task1_4['entries']
        #checking for duplicates
        duplicate = False
        with open(file2) as json_file1:
            ...
            for entry in data2['entries']:
                temp.append(entry)
    write_json(data_task1_4,"data_task1_4.json")
...
#joinDataFiles("data_task1_4.json","2020.json")
